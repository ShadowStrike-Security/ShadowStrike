...
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│                                    PRESENTATION LAYER                                    │
│                              (GUI - Qt/WinUI - NOT YET IMPLEMENTED)                      │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                    APPLICATION LAYER                                     │
│                            (Core Engine - NOT YET IMPLEMENTED)                           │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐  │
│  │ ScanEngine   │ │ BehaviorMon  │ │ RealTimeProt │ │ NetworkMon   │ │ QuarantineMgr│  │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘  │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                    DATA ACCESS LAYER                                     │
│                              (INFRASTRUCTURE - IMPLEMENTED ✓)                            │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐  │
│  │SignatureStore│ │  HashStore   │ │ PatternStore │ │ ThreatIntel  │ │  Whitelist   │  │
│  │   (mmap)     │ │   (mmap)     │ │   (mmap)     │ │   (mmap)     │ │ (mmap+SQLite)│  │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘  │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                    PERSISTENCE LAYER                                     │
│                              (INFRASTRUCTURE - IMPLEMENTED ✓)                            │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐                    │
│  │ ConfigDB     │ │   LogDB      │ │ QuarantineDB │ │   Utils      │                    │
│  │  (SQLite)    │ │  (SQLite)    │ │  (SQLite)    │ │   Library    │                    │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘                    │
└─────────────────────────────────────────────────────────────────────────────────────────┘
```
...
|-----------|--------|---------------|-------|
| SignatureStore | ✅ Complete | ~25,000 | Memory-mapped, B+Tree, YARA |
| HashStore | ✅ Complete | ~8,000 | Bloom filter, hash tables |
| PatternStore | ✅ Complete | ~6,000 | Aho-Corasick, Boyer-Moore, SIMD |
| ThreatIntel | ✅ Complete | ~20,000 | IOC management, feed parsing |
| Whitelist (MM) | ✅ Complete | ~12,000 | Memory-mapped, B+Tree |
| Database (SQLite) | ✅ Complete | ~8,000 | Config, Log, Quarantine |
| Utils | ✅ Complete | ~30,000 | Crypto, Network, Hash, etc. |
| **Core Engine** | 🔴 Not Started | - | Scan engine, behavior analysis |
| **GUI** | 🔴 Not Started | - | Qt/WinUI interface |
...
│   │
│   ├── SignatureStore/             # Malware signature storage (mmap)
│   │   ├── SignatureStore.hpp/cpp  # Main store interface
│   │   ├── SignatureIndex.hpp/cpp  # B+Tree index implementation
│   │   ├── SignatureFormat.hpp/cpp # Binary format definitions
│   │   ├── SignatureBuilder.hpp/cpp# Offline signature compiler
│   │   ├── YaraRuleStore.hpp/cpp   # YARA rule integration
│   │   └── [implementation files]
│   │
│   ├── HashStore/                  # Hash-based detection (mmap)
│   │   ├── HashStore.hpp/cpp       # Hash storage interface
│   │   ├── BloomFilter_impl.cpp    # Bloom filter implementation
│   │   ├── HashBucket_impl.cpp     # Hash bucket management
│   │   └── [query/import/export]
│   │
│   ├── PatternStore/               # Pattern matching (mmap)
│   │   ├── PatternStore.hpp/cpp    # Pattern storage interface
│   │   ├── PatternIndex.cpp        # Pattern indexing
│   │   ├── aho_crsck_impl.cpp      # Aho-Corasick algorithm
│   │   ├── boyer_moore_impl.cpp    # Boyer-Moore algorithm
│   │   └── SIMD_matcher_impl.cpp   # AVX2/SSE4 optimized matching
│   │
│   ├── ThreatIntel/                # Threat intelligence (mmap)
│   │   ├── ThreatIntelFormat.hpp   # IOC format definitions
│   │   ├── ThreatIntelDatabase.hpp/cpp
│   │   ├── ThreatIntelFeedManager.hpp/cpp
│   │   ├── ThreatIntelIOCManager.cpp
│   │   ├── ReputationCache.hpp/cpp # File reputation caching
│   │   └── [index/importer/exporter]
│   │
│   ├── Whitelist/                  # Whitelist system (mmap)
│   │   ├── WhiteListStore.hpp/cpp  # Main whitelist interface
│   │   ├── WhiteListFormat.hpp     # Binary format for mmap
│   │   ├── WhiteListHashIndex.cpp  # Hash-based B+Tree index
│   │   ├── WhiteListPathIndex.cpp  # Path trie index
│   │   ├── WhiteListBloomFilter.cpp
│   │   └── WhiteListStringPool.cpp
│   │
│   ├── Database/                   # SQLite databases
│   │   ├── DatabaseManager.hpp/cpp # SQLite wrapper
│   │   ├── ConfigurationDB.hpp/cpp # Settings storage
│   │   ├── LogDB.hpp/cpp           # Audit logging
│   │   └── QuarantineDB.hpp/cpp    # Quarantine metadata
│   │
│   └── Utils/                      # Utility library
│       ├── Logger.hpp/cpp          # Logging system
│       ├── ThreadPool.hpp/cpp      # Thread pool
│       ├── CryptoUtils.hpp/cpp     # Cryptographic functions
│       ├── HashUtils.hpp/cpp       # Hash calculations
│       ├── NetworkUtils.hpp/cpp    # Network operations
│       ├── FileUtils.hpp/cpp       # File operations
│       ├── StringUtils.hpp/cpp     # String manipulation
│       └── [other utilities]
│
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         SIGNATURESTORE SYSTEM                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                      SignatureStore (Main Interface)                 │   │
│   │  - Open/Close signature databases                                    │   │
│   │  - Lookup signatures by hash/pattern                                 │   │
...
│                    ▼                               ▼                        │
│   ┌─────────────────────────┐     ┌─────────────────────────┐              │
│   │    SignatureIndex       │     │    YaraRuleStore        │              │
│   │  (B+Tree Implementation)│     │  (YARA Rule Engine)     │              │
│   │  - O(log n) lookups     │     │  - Rule compilation     │              │
│   │  - Copy-on-write        │     │  - Pattern matching     │              │
│   │  - Cache-friendly       │     │  - Memory-mapped rules  │              │
│   └─────────────────────────┘     └─────────────────────────┘              │
│                    │                               │                        │
│                    ▼                               ▼                        │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    Memory-Mapped File (.ssig)                        │   │
│   │  ┌──────────┬───────────┬───────────┬──────────┬─────────────────┐  │   │
│   │  │  Header  │  B+Tree   │  String   │   YARA   │   Signature     │  │   │
│   │  │  (4KB)   │  Index    │   Pool    │  Rules   │     Data        │  │   │
│   │  └──────────┴───────────┴───────────┴──────────┴─────────────────┘  │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
| Pattern match | < 1μs | ~500ns |
| B+Tree search | O(log n) | ✓ |
| Memory usage | Minimal (mmap) | ✓ |
...
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                        HashStore (Main Interface)                    │   │
│   │  - Add/Remove/Query hashes                                           │   │
...
│              ▼                     ▼                     ▼                  │
│   ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐          │
│   │  Bloom Filter   │   │   Hash Buckets  │   │  Query Cache    │          │
│   │  (Pre-filter)   │   │  (Actual Data)  │   │   (LRU)         │          │
│   │  ~20ns lookup   │   │  ~50ns lookup   │   │  ~10ns lookup   │          │
│   │  99.99% reject  │   │  O(1) access    │   │  Hot entries    │          │
│   └─────────────────┘   └─────────────────┘   └─────────────────┘          │
│                                    │                                         │
│                                    ▼                                         │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    Memory-Mapped File (.shash)                       │   │
│   │  ┌──────────┬───────────────┬────────────────┬────────────────────┐ │   │
│   │  │  Header  │ Bloom Filter  │  Hash Buckets  │   Metadata         │ │   │
│   │  │  (4KB)   │   (1MB)       │  (variable)    │                    │ │   │
│   │  └──────────┴───────────────┴────────────────┴────────────────────┘ │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
...
```
Query Hash → Cache Check → Bloom Filter → Hash Bucket → Result
              (10ns)        (20ns)         (50ns)
...
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                      PatternStore (Main Interface)                   │   │
│   │  - Add/Remove/Query patterns                                         │   │
...
│          ▼                         ▼                         ▼              │
│   ┌──────────────┐       ┌──────────────┐       ┌──────────────┐           │
│   │ Aho-Corasick │       │ Boyer-Moore  │       │ SIMD Matcher │           │
│   │  (Multi)     │       │  (Single)    │       │  (AVX2/SSE)  │           │
│   │ O(n+m+k)     │       │ O(n/m)       │       │ ~4GB/s       │           │
│   └──────────────┘       └──────────────┘       └──────────────┘           │
│          │                         │                         │              │
│          └─────────────────────────┼─────────────────────────┘              │
...
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                   ThreatIntelDatabase (Main Interface)               │   │
│   │  - IOC management (add/remove/query)                                 │   │
│   │  - Feed integration (STIX, OpenIOC, MISP)                           │   │
│   │  - Reputation scoring                                                │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                    │                         │                               │
│                    ▼                         ▼                               │
│   ┌───────────────────────────┐ ┌───────────────────────────┐              │
│   │   ThreatIntelFeedManager  │ │      ReputationCache      │              │
│   │  - Feed parsing           │ │  - File reputation        │              │
│   │  - STIX 2.1 support      │ │  - URL reputation         │              │
│   │  - Auto-update           │ │  - IP reputation          │              │
│   └───────────────────────────┘ └───────────────────────────┘              │
│                    │                         │                               │
│                    ▼                         ▼                               │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    ThreatIntelIndex (B+Tree)                         │   │
│   │  - IPv4/IPv6 indexing                                                │   │
│   │  - Domain indexing                                                   │   │
│   │  - Hash indexing                                                     │   │
│   │  - URL indexing                                                      │   │
...
│                                    ▼                                         │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    Memory-Mapped File (.sstioc)                      │   │
│   │  ┌────────┬─────────┬─────────┬──────────┬─────────┬──────────────┐ │   │
│   │  │ Header │ IP Index│ Domain  │ Hash     │ String  │   IOC        │ │   │
│   │  │ (4KB)  │ (B+Tree)│ Index   │ Index    │ Pool    │   Entries    │ │   │
│   │  └────────┴─────────┴─────────┴──────────┴─────────┴──────────────┘ │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    WHITELIST DUAL-LAYER ARCHITECTURE                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                         USE CASE COMPARISON                          │   │
│   ├─────────────────────────────┬─────────────────────────────────────────┤   │
│   │   SQLite WhitelistDB        │    Memory-Mapped WhitelistStore        │   │
│   ├─────────────────────────────┼─────────────────────────────────────────┤   │
│   │ ✓ Admin adds entry via GUI │ ✓ Real-time file access check          │   │
│   │ ✓ Complex SQL queries       │ ✓ Kernel-mode driver lookup            │   │
│   │ ✓ Reporting & analytics     │ ✓ Millions of checks/second           │   │
│   │ ✓ Import/Export (CSV/Excel) │ ✓ Sub-microsecond response             │   │
│   │ ✓ Audit logging             │ ✓ Zero-copy reads                      │   │
│   │ ✗ ~1-10ms lookup speed     │ ✗ Limited query flexibility            │   │
│   │ ✗ Not for real-time         │ ✗ Read-optimized (slow writes)        │   │
│   └─────────────────────────────┴─────────────────────────────────────────┘   │
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                         DATA FLOW                                    │   │
│   │                                                                      │   │
│   │   Admin Dashboard ──┬──► SQLite DB ──► Sync Manager ──► MM Store    │   │
│   │                     │       │                              │          │   │
│   │   API/CLI ──────────┘       │                              │          │   │
│   │                             ▼                              ▼          │   │
│   │                       whitelist.db              whitelist.mmdb       │   │
│   │                       (management)              (runtime)            │   │
│   │                                                     │                │   │
│   │   Scan Engine ◄──────────────────────────────────────┘                │   │
│   │   Kernel Driver ◄────────────────────────────────────┘                │   │
...
Offset 0x1000: ┌──────────────────────────────────────────────────────────────┐
               ...
               ┌──────────────────────────────────────────────────────────────┐
               │                    Entry Data                                 │
               │  - WhitelistEntry structures (128 bytes each)                 │
               │  - Cache-line aligned                                         │
               └──────────────────────────────────────────────────────────────┘
               ┌──────────────────────────────────────────────────────────────┐
               │                    String Pool                                │
...
```
Query → Cache Check → Bloom Filter → B+Tree Lookup → Entry Validation → Result
          (~30ns)      (~20ns)        (~40ns)           (~10ns)
...
│              ▼                    ▼                    ▼                     │
│   ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐              │
│   │ ConfigurationDB │ │     LogDB       │ │  QuarantineDB   │              │
│   │                 │ │                 │ │                 │              │
│   │ - Settings      │ │ - Scan logs     │ │ - Quarantine    │              │
│   │ - Policies      │ │ - Threat logs   │ │   metadata      │              │
│   │ - Profiles      │ │ - System events │ │ - File paths    │              │
│   │ - Schedules     │ │ - Audit trail   │ │ - Timestamps    │              │
│   └─────────────────┘ └─────────────────┘ └─────────────────┘              │
│              │                    │                    │                     │
│              ▼                    ▼                    ▼                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    SQLite Database Files                             │   │
│   │    config.db          logs.db           quarantine.db               │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
Level 1: Global Store Lock (std::shared_mutex)
    │
    └─► Level 2: Section Locks (per B+Tree node)
            │
            └─► Level 3: Entry Locks (Interlocked for counters)
```
...
| Pattern match | < 1μs | Aho-Corasick |
...
Core/
├── Engine/
│   ├── ScanEngine.hpp/cpp        # Main scan orchestrator
│   ├── ThreatDetector.hpp/cpp    # Detection logic
│   ├── QuarantineManager.hpp/cpp # Quarantine operations
│   ├── HeuristicAnalyzer.hpp/cpp # Heuristic detection
│   ├── BehaviorAnalyzer.hpp/cpp  # Behavior monitoring
│   ├── EmulationEngine.hpp/cpp   # Code emulation
│   └── SandboxAnalyzer.hpp/cpp   # Sandbox analysis
│
├── RealTime/
│   ├── RealTimeProtection.hpp/cpp
│   ├── FileSystemFilter.hpp/cpp  # Minifilter driver
│   └── ProcessMonitor.hpp/cpp
│
├── Network/
│   ├── NetworkMonitor.hpp/cpp
│   ├── FirewallManager.hpp/cpp
│   └── URLAnalyzer.hpp/cpp
│
└── Service/
    ├── AntivirusService.hpp/cpp  # Windows service
    └── ServiceController.hpp/cpp
```
...
```
          ...
          ▼                      ▼                      ▼
┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐
│ SignatureStore  │   │    HashStore    │   │   ThreatIntel   │
...
public:
    // Sync SQLite → Memory-Mapped (startup or manual)
    StoreError SyncToRuntime();
    
    // Sync Memory-Mapped → SQLite (statistics, hit counts)
